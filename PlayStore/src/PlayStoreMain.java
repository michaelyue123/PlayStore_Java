...
public class PlayStoreMain {
	...
	// create a simple menu for code reuse   
	...
	// displays all comments by iterating through comment list
	public static void displayComment(PlayStore store, String contentID)
	{
		ArrayList<Comment> comments = store.contents.get(contentID).showComments();	
  		...
  		for(int i=0;i<comments.size();i++)
  			System.out.println(comments.get(i).user.name + "  :  " + comments.get(i).comment);
	}
	...
	public static void main(String[] args) {    	
        ...
        // add content(publication) to store        
        store.addContent(b1);
        store.addContent(b2);
        store.addContent(b3);
        store.addContent(m1);
        ...
        // add content(application) to store     
        store.addContent(g1);
        store.addContent(g2);
        ...
        // new users
        User u1 = new User("u1", "John Doe", "0412000", 500);
        ...
        // remove comment from different content types
        ...
        // user1's specific conditions
        u1.buyContent(b1);
        u1.buyContent(b3);
        u1.buyContent(m1);

        // user4's specific conditions
        u4.buyContent(g1);
        u4.becomePremium();
        u4.buyContent(m1);

        // user2's specific conditions
        u2.becomePremium();
        u2.buyContent(b1);
        u2.buyContent(g1);
        ...
        // menu driven program
  	    ...
  	    while(true) 
  	    {
  	    	...
  	    	choice = input.nextLine();	// get choice
  	    	try
  	    	{
  	    		option = Integer.parseInt(choice);
  	    	}
  	    	catch(Exception e)	// in case of invalid inputs
  	    	{
  	    		System.out.println("Invalid Input. \nChoose an option from the Menu below: ");
  		        continue;
  	    	}
  	    	// condition should be the option should be between 1 and 7
  		    ...
  		    switch (option) {
  		    	...
  		    	case 1:  //Upgrading a member to premium account
  		        	System.out.println("Please enter the user ID: ");
  		        	userID = input.nextLine();		// get user id
  		        	if(store.users.containsKey(userID))	// validation
  		        		store.users.get(userID).becomePremium();
  		        	else
  		        		System.out.println("Invalid User ID");	
  		        	break;
  		        	
  		    	case 2:  //Purchasing one item for one user
  		        	System.out.println("Please enter the user ID: ");
  		        	userID = input.nextLine();	// get user ID
  		        	System.out.println("Please enter the content ID: ");
  		        	contentID = input.nextLine();	// get content ID
  		        	// validation
  		    	...
  		    	case 3:	//Listing all available contents
  		            store.showContent();
  		            break;
  		            
  		    	case 4:	// Showing all purchased items of a user
  		    		System.out.println("Please enter the user ID: ");
  		        	userID = input.nextLine();			// get user ID
  		        	if(store.users.containsKey(userID))			// validation
  		    	...
  		    	case 5:	// Showing all comments of a content
  		    		System.out.println("Please enter the content ID: ");
  		    		contentID = input.nextLine();	// get content ID
  		        	if(!store.contents.containsKey(contentID))	// validate
  		        		System.out.println("Invalid Content ID");
  		        	else
  		        		displayComment(store,contentID);
  		        	break;
  		    	case 6:	// Showing all content items of a particular type
  		    		System.out.println("1.Book\n2.Magazine\n3.Application\nChoose a Type");
  		    		choice = input.nextLine();	// choose type
  		  	    	try
  		  	    	{
  		  	    		int opt = Integer.parseInt(choice);	// parse to integer
  		  	    		if(opt == 1)
  		  	    		...
  		  	    		else		// invalid choice throw exception
  		  	    			throw new Exception();
  		  	    	}
  		  	    	catch(Exception e)
  		  	    	{
